...
essays_train = train['essay'].tolist()[0:100]
essays_test = test['essay'].tolist()
scores_train = train['domain1_score'].tolist()[0:100]
...
df = df.transpose()
df.to_csv('hclust100Ridge.csv',sep=',')
...
print ("training done")
joblib.dump(reg, 'Regression100Ridge.pkl') 

#reg = joblib.load('Regression30.pkl') 

test_essay = essays_test[0:50]
for i in range(0,len(test_essay)):
    fin_predict = sentence_similarity(mSynset,test_essay[i])
    predicted = reg.predict([fin_predict])
    #print ("predicted score")
    print(str(predicted[0]) + " " + str(scores_test[i]))
    #prin	t ("actual score")
    print("")
